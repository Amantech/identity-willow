...
<!-- 	<link rel="icon" sizes="192x192" href="img/icons/willow_192x192.jpg"> -->
...
<!-- 	<meta name="msapplication-TileImage" content="img/icons/willow_144x144.png"> -->
<!-- 	<meta name="msapplication-TileColor" content="#ee3366"> -->
...
.willow .mdl-card__media a, .willow .mdl-card__title a {
	color: inherit;
}
...
.willow:not (.willow--blogpost ) .mdl-card__supporting-text ~
...
.willow--blogpost .comments>form .mdl-textfield input, .willow--blogpost .comments>form .mdl-textfield textarea
	{
	resize: none;
}
...
.social-btn__gplus {
	background-image:
		url('https://www.gstatic.com/images/icons/material/system/2x/post_gplus_black_24dp.png');
}
...
<script>
	...
	function showPosition(position) {
		...
		var smhiAPI = "http://opendata-download-metfcst.smhi.se/api/category/pmp1.5g/version/1/geopoint/lat/"
				...
				.each(
						...
						function(i, item) {
							...
							if (new Date(item.validTime) > nu) {
								...
								var time = $("<td/>").html(t);
								var temperature = $("<td/>").html(
										getTemp(item.t));
								var windDirection = $("<td/>").html(
										getWindDirection(item.wd));
								var cloud = $("<td/>").html(getCloud(item.tcc));
								var precipitation = $("<td/>").html(
										getPrecipitation(item));
								weatherDataRow.append(time).append(temperature)
										.append(windDirection).append(cloud)
	...
	function getCloud(n) {
		...
		return "<i title='" + n + "' style='font-size:1.5em;color:" + c + "' class='material-icons'>cloud</i>";
	}
	...
	function getTemp(n) {
		...
		return "<div style='display:inline-block; width:2em; color:" + color + "'>"
				+ t + "</div>"
	...
	function getWindDirection(n) {
		...
		return "<div class='wind-dir' title='"
				+ n
				+ "' style='height:1em;display:inline-block;font-size:1.5em; line-height:0.9em; margin-right:0.3em;transform:rotate("
				+ n + "deg);-webkit-transform:rotate(" + n
				+ "deg)'>&darr;</div><span>" + dir + "</span>";
	}
